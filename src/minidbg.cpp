    ...
    if (die.tag == DW_TAG::variable) {
      ...
      if (loc_val.get_type() == value::type::exprloc) {
        ...
        case expr_result::type::reg: {
          auto value =
              get_register_value_from_dwarf_register(m_pid, result.value);
...
void debugger::initialise_load_address() {
  ...
  if (m_elf.get_hdr().type == elf::et::dyn) {
    ...
    try {
      ...
      // Handle error, e.g., skip this conversion or set a default value
    } catch (const std::invalid_argument &e) {
      std::cerr << "Invalid argument: " << e.what() << '\n';
      // Handle the case where no conversion could be performed
    }
  }
}
...
void debugger::handle_sigtrap(siginfo_t info) {
  ...
  case TRAP_BRKPT: {
    ...
    std::cout << "Hit breakpoint at address 0x" << std::hex << get_pc()
              << std::endl;
    auto offset_pc = offset_load_address(
        get_pc()); // rember to offset the pc for querying DWARF
